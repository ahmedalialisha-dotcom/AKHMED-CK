...
export default function OpenWorldMode(props: Props) {
  ...
  useOpenWorldScene(mountRef, position, props.hairStyle, props.equippedItems, cutting, eating, driving);

  useEffect(() => {
    const down = (event: KeyboardEvent) => keys.current.add(event.code);
    const up = (event: KeyboardEvent) => keys.current.delete(event.code);
    const release = () => { keys.current.clear(); movementSpeed.current = 0; };
    ...
    const timer = window.setInterval(() => {
      setPosition((current) => {
        ...
        const desiredSpeed = cutting || eating ? 0 : direction * maximumSpeed;
    ...
    return () => { window.clearInterval(timer); window.removeEventListener("keydown", down); window.removeEventListener("keyup", up); window.removeEventListener("blur", release); };
  }, [cutting, driving, eating]);
  ...
  const chooseHair = (style: HairStyle) => {
    ...
    actionTimer.current = window.setTimeout(() => { props.onHair(style); setCutting(false); }, 2600);
  };
  const interact = () => {
    if (nearby?.id === "cafe" && props.onEat()) {
      setPosition({ x: -18, z: -14, heading: 0 }); setEating(true);
      actionTimer.current = window.setTimeout(() => { setHunger(100); setEating(false); }, 2200);
    }
    else if (nearby?.id === "barber") setChoosingHair(true);
    else if (nearby) {
      setEnteringShop(true);
      actionTimer.current = window.setTimeout(() => { setEnteringShop(false); props.onOpenShop(); }, 650);
    }
  };
    ...
    <main className="open-world">
      ...
      <div className="world-layout">
        <section className="world-stage">
          ...
          {nearby && !cutting && !eating && <button className="world-interact" onClick={interact}>{nearby.id === "cafe" ? "Поесть · 30 ●" : nearby.id === "barber" ? "Выбрать стрижку ✂️" : `Войти: ${nearby.name}`}</button>}
          ...
          {eating && <div className="world-action eating-action"><b>🍔</b><span>Приятного аппетита!</span><i /></div>}
          {enteringShop && <div className="shop-transition"><b>Добро пожаловать</b><span>Открываем двери магазина…</span></div>}
        </section>
