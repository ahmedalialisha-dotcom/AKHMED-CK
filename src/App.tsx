...
export default function App() {
  ...
  if (screen === 'careerPlay') return <CareerMode player={player} team={selectedTeam} onExit={() => setScreen('menu')} />;
  ...
  return <MainMenu screen={menuScreen} player={player} selectedTeam={selectedTeam} opponentTeam={opponentTeam} userEmail={user?.email} onStart={(mode, home = selectedTeam, away = opponentTeam) => { setTournament(mode); setMatchTeams([home, away]); setScreen('match'); }} onPenalty={(home, away) => { setMatchTeams([home, away]); setScreen('penalty'); }} onCareer={() => setScreen('careerPlay')} onTournament={(name) => { setTournament(name); setScreen('tournament'); }} onScreen={setScreen} onPlayer={setPlayer} onTeam={setSelectedTeam} onOpponent={setOpponentTeam} />;
}
