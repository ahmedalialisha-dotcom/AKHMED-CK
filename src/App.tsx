...
export default function App() {
  const [screen, setScreen] = useState<'menu' | 'match' | 'penalty' | 'career' | 'settings' | 'tournament' | 'auth' | 'profile'>('menu');
  ...
  return <MainMenu screen={menuScreen} player={player} userEmail={user?.email} onStart={(mode) => { setTournament(mode); setScreen('match'); }} onTournament={(name) => { setTournament(name); setScreen('tournament'); }} onScreen={setScreen} onPlayer={setPlayer} />;
}
