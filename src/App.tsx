...
export default function App() {
  const [screen, setScreen] = useState<'menu' | 'quick' | 'penaltySelect' | 'match' | 'penalty' | 'career' | 'settings' | 'tournament' | 'auth' | 'profile'>('menu');
  ...
  return <MainMenu screen={menuScreen} player={player} selectedTeam={selectedTeam} opponentTeam={opponentTeam} userEmail={user?.email} onStart={(mode, home = selectedTeam, away = opponentTeam) => { setTournament(mode); setMatchTeams([home, away]); setScreen('match'); }} onPenalty={(home, away) => { setMatchTeams([home, away]); setScreen('penalty'); }} onTournament={(name) => { setTournament(name); setScreen('tournament'); }} onScreen={setScreen} onPlayer={setPlayer} onTeam={setSelectedTeam} onOpponent={setOpponentTeam} />;
}
