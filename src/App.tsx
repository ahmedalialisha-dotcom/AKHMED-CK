...
export default function App() {
  ...
  if (screen === 'profile' && user) return <main className="auth-page"><button className="auth-page__back" onClick={() => setScreen('menu')}>← В главное меню</button><div className="auth-page__intro"><p>ТВОЙ ПРОФИЛЬ</p><h1>{user.email}</h1><button onClick={() => supabase.auth.signOut()}>Выйти из аккаунта</button></div></main>;
